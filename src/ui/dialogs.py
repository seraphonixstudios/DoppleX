...
def show_error(page: ft.Page, title: str, message: str, trace: str | None = None) -> None:
    ...
    try:
        trace_area = ft.TextArea(label="Trace", value=trace or "", height=180, multiline=True, visible=False)
        def _close(_):
            page.dialog = None
            page.update()
        ...
        if hasattr(page, "show_dialog"):
            ...
            from ft import Snackbar  # type: ignore
            page.snackbar = Snackbar(ft.Text(f"{title}: {message}"))
            page.update()
        page.update()
    except Exception:
